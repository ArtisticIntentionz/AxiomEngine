...
# --- Mocking spaCy ---
# We use a more robust mock that can check for similarity in both directions.
# This makes the tests less brittle if the application logic changes.
class MockSpacyDoc:
...
class TestVerificationEngine(unittest.TestCase):
    ...
    def test_find_corroborating_claims_success(self):
        ...
        all_facts = [corroborating_fact, unrelated_fact]
        self.mock_session.query(Fact).filter().all.return_value = all_facts
    ...
    def test_verify_citations(self, mock_requests_head):
        ...
        def side_effect(url, **kwargs):
            ...
            if url == "http://good-url.com":
                ...
                response.status_code = 500 # Default for unexpected URLs
            return response
