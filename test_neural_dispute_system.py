...
from axiom_server.dispute_system import (
    DisputeEvidence,
    DisputeSystem,
)
from axiom_server.enhanced_fact_processor import EnhancedFactProcessor
from axiom_server.ledger import ENGINE, Base
    ...
    test_facts = [
        {
            ...
            "sources": [
                {"domain": "nasa.gov", "url": "https://nasa.gov/solar-system"},
            ],
        ...
        },
        {
            ...
            "sources": [
                {"domain": "nature.com", "url": "https://nature.com/articles"},
            ],
            ...
            "sources": [
                {
                    ...
                    "url": "https://conspiracy-blog.com/aliens",
                },
            ],
...
def test_dispute_system():
    ...
    logger.info(
        f"Dispute Summary: {json.dumps(summary, indent=2, default=str)}",
    )
...
def test_enhanced_fact_processor():
    ...
    test_facts = [
        ...
        },
        {
            ...
            "sources": [
                ...
                {
                    "domain": "science.org",
                    "url": "https://science.org/quantum-advantage",
                },
            ],
            "metadata": {
                "category": "technology",
        ...
        },
        {
            ...
            "sources": [
                {
                    "domain": "cryptid-news.com",
                    "url": "https://cryptid-news.com/bigfoot",
                },
            ],
            "metadata": {
    ...
    for i, test_fact in enumerate(test_facts):
        ...
        logger.info(
            f"  - Neural Confidence: {result['neural_verification']['confidence']:.3f}",
        )
        logger.info(
            f"  - Overall Confidence: {result['enhanced_analysis']['overall_confidence']:.3f}",
        )
        ...
        if result.get("dispute_created"):
            logger.info(
                f"  - Auto-dispute Created: {result['dispute_created']['dispute_id']}",
            )
...
def test_neural_training():
    ...
    logger.info(
        f"Training neural network with {len(training_data)} examples...",
    )
    ...
    for content, label in training_data:
        logger.info(
            f"  - {content} -> {label} ({'True' if label else 'False'})",
        )
...
def test_integration():
    ...
    result = processor.process_fact(
        ...
        sources=[
            {
                "domain": "conspiracy-forum.com",
                "url": "https://conspiracy-forum.com/time-travel",
            },
        ],
    ...
    if result.get("dispute_created"):
        logger.info(
            f"  - Auto-dispute created successfully: {result['dispute_created']['dispute_id']}",
        )
        ...
        logger.info(
            "  - No auto-dispute created (confidence may be above threshold)",
        )
...
def main():
    ...
    try:
        ...
        logger.info(
            "The neural network verification and dispute system is working correctly.",
        )
