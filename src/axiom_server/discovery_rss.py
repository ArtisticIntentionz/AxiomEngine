...
def get_content_from_prioritized_feed(
    ...
    for feed_url in shuffled_feeds:
        ...
        try:
            ...
            if content_list:
                ...
                logger.info(f"Feed {feed_url} was valid but contained no new items. Trying next.")
    ...
    logger.error("Failed to retrieve content from ANY of the configured RSS feeds.")
    return []
...
def get_all_headlines_from_feeds() -> List[str]:
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        ...
        future_to_url = {
            executor.submit(_fetch_one_feed_headlines, url): url for url in unique_feed_urls
        }
        ...
        for future in as_completed(future_to_url):
            try:
                ...
                logger.warning(f"Concurrent fetch for {url} generated an exception: {exc}")

    logger.info(f"Fetched a total of {len(all_headlines)} headlines concurrently.")
