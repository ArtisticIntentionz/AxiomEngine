...
from .ledger import Block, Fact, SerializedFact, Source, get_latest_block
...
def sync_with_peer(node_instance: AxiomNode, peer_url: str, session: Session) -> tuple[str, int]:
    """
    ...
    try:
        ...
        for block_data in blocks_data:
            # Reconstruct the block object to validate its hash.
            new_block = Block(
            ...
            # --- THE CORE CONSENSUS RULE ---
            # Cryptographically validate the block before accepting it.
            ...
            if current_valid_block and new_block.previous_hash != current_valid_block.hash:
        ...
        local_fact_hashes: set[str] = {fact.hash for fact in session.query(Fact).with_entities(Fact.hash)}
        ...
        logger.warning(f"Connection failed during sync with {peer_url}. Error: {e}")
