...
from .ledger import Block, Fact, get_latest_block, SerializedFact, Source
...
def sync_with_peer(
    node_instance: AxiomNode, peer_url: str, session: Session
) -> tuple[str, int]:
    """
    ...
    try:
        ...
        response = requests.get(
            f"{peer_url}/get_blocks?since={local_height}", timeout=30
        )
        response.raise_for_status()
