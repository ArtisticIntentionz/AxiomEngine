...
from axiom_server.p2p.constants import (
    BOOTSTRAP_IP_ADDR, BOOTSTRAP_PORT, ENCODING, KEY_SIZE, NODE_BACKLOG,
    NODE_CERT_FILE, NODE_CHECK_TIME, NODE_CHUNK_SIZE, NODE_CONNECTION_TIMEOUT,
    NODE_KEY_FILE, SEPARATOR, SIGNATURE_SIZE,
)
...
class RawMessage(BaseModel):
    ...
    def from_bytes(data: bytes) -> RawMessage:
        return RawMessage(signature=data[:SIGNATURE_SIZE], data=data[SIGNATURE_SIZE:])
    def check_signature(self, public_key: rsa.RSAPublicKey) -> bool:
        return _verify(self.signature, self.data, public_key)

class MessageType(Enum):
    PEERS_REQUEST, PEERS_SHARING, APPLICATION, GET_CHAIN, CHAIN_RESPONSE = range(5)
...
class Message(BaseModel):
    ...
    def _from_bytes(data: bytes) -> "Message":
        try:
            ...
            raise P2PRuntimeError(f"cannot create Message from bytes ({e})") from e
    @staticmethod
    def from_raw(raw: RawMessage) -> "Message":
        return Message._from_bytes(raw.data)
    def check_content(self) -> bool:
        return (
            (self.message_type == MessageType.PEERS_REQUEST and isinstance(self.content, PeersRequest)) or
            (self.message_type == MessageType.PEERS_SHARING and isinstance(self.content, PeersSharing)) or
            (self.message_type == MessageType.APPLICATION and isinstance(self.content, ApplicationData)) or
            (self.message_type == MessageType.GET_CHAIN and isinstance(self.content, MessageContent))
        )
    @staticmethod
    ...
    def peers_sharing(peers: list["Peer"]) -> "Message":
        return Message(
            ...
            content=PeersSharing(peers=[p.to_serialized() for p in peers if p.can_be_shared()])
        )
    @staticmethod
    ...
    def get_chain_request() -> "Message":
        return Message(message_type=MessageType.GET_CHAIN, content=MessageContent())

class SerializedPeer(BaseModel):
    ip_address: str
    port: int
    def to_peer(self) -> "Peer":
        return Peer(ip_address=self.ip_address, port=self.port, public_key=None)
...
def _verify(signature: bytes, message: bytes, public_key: rsa.RSAPublicKey) -> bool:
    try:
...
class NodeContextManager:
    node: "Node"
    def __enter__(self) -> "Node": return self.node
    def __exit__(self, exc_type, exc_value, exc_tb): self.node.stop()

def _all(item: Any) -> Literal[True]: return True
ALL = _all
...
class Node:
    ...
    def start(ip_address: str, port: int = 0, public_ip: str | None = None) -> "Node":
        ...
        if not NODE_CERT_FILE.exists(): raise P2PRuntimeError(f"cannot load cert file {NODE_CERT_FILE}")
        if not NODE_KEY_FILE.exists(): raise P2PRuntimeError(f"cannot load key file {NODE_KEY_FILE}")
        ...
        return Node(
            ip_address=computed_ip_address, port=computed_port, public_ip=final_public_ip,
            serialized_port=str(computed_port).encode(ENCODING), private_key=private_key,
            public_key=public_key, serialized_public_key=_serialize_public_key(public_key),
    ...
    def stop(self) -> None:
        for link in self.peer_links:
            if link.alive: link.socket.close()
        self.server_socket.close()
        logger.info("closed server socket")

    def update(self) -> None:
        sockets: list[Socket] = [self.server_socket] + [p.socket for p in self.peer_links]
        try:
            ...
            if self.server_socket in readable:
                try:
                    ...
                    if "HTTP_REQUEST" in str(e):
                        logger.warning("Ignored a plain HTTP request on the secure P2P port.")
            ...
            for link in self.peer_links:
                if link.socket in readable:
                    try: self._recv(link)
                        ...
                        logger.exception(f"{link.fmt_addr()} error receiving: {e}")
                    if not link.alive:
    ...
    def bootstrap(self, ip_addr: str = BOOTSTRAP_IP_ADDR, port: int = BOOTSTRAP_PORT) -> bool | None:
        ...
        if self._is_self(ip_addr, port):
            ...
            return True
        link = self.search_link_by_peer(lambda p: p.ip_address == ip_addr and p.port == port)
        if link is None:
            link = self._create_link(ip_addr, port)
            if link is None:
                logger.error("Failed to bootstrap: can't connect to server")
                return None
            logger.info(f"Connection to {link.fmt_addr()} successful. Requesting data...")
    ...
    def _connect_to_peer(self, ip_address: str, port: int) -> Socket | None:
        if self._is_self(ip_address, port): return None
    ...
    def _handle_new_connection(self, socket: Socket, addr: socket_lib._RetAddress) -> None:
        if socket.family != socket_lib.AF_INET:
            logger.info(f"{addr} ignoring non-INET socket: {socket.family}")
            return
        ip_addr, port = addr
        link = PeerLink(peer=Peer(ip_address=ip_addr, port=None, public_key=None), socket=socket, alive=True, buffer=b"")
    ...
    def _handle_public_key_declaration(self, link: PeerLink) -> bool:
        if link.peer.public_key is None:
            try: key = _deserialize_public_key(link.buffer)
            except P2PRuntimeError as e:
    ...
    def _handle_port_declaration(self, link: PeerLink) -> bool:
        if link.peer.port is None:
            try: port = int(link.buffer.decode(ENCODING))
    ...
    def _handle_peers_sharing(self, link: PeerLink, content: PeersSharing) -> None:
        ...
        for serialized_peer in content.peers:
            ...
            if self._is_self(shared_peer.ip_address, shared_peer.port):
                continue
            if self.search_link_by_peer(lambda p: p.ip_address == shared_peer.ip_address and p.port == shared_peer.port):
                continue
    ...
    def _create_link(self, ip_address: str, port: int) -> PeerLink | None:
        ...
        if socket:
            link = PeerLink(peer=Peer(ip_address=ip_address, port=port, public_key=None), socket=socket, alive=True, buffer=b"")
            self.peer_links.append(link)
    ...
    def _handle_buffer_readable(self, link: PeerLink) -> None:
        ...
        if not raw_message.check_signature(link.peer.public_key):
            logger.error(f"{link.fmt_addr()} ignoring message: invalid signature")
            return
        message = Message.from_raw(raw_message)
        if not message.check_content():
            logger.error(f"{link.fmt_addr()} ignoring message: content/type mismatch")
            return
    ...
    def _handle_message(self, link: PeerLink, message: Message):
        if message.message_type == MessageType.GET_CHAIN:
            if hasattr(self, "get_chain_callback") and callable(self.get_chain_callback):
                logger.info(f"Peer {link.fmt_addr()} requested blockchain. Executing callback...")
                chain_data_json = self.get_chain_callback()
                self._send_message(link, Message.application_data(chain_data_json))
    ...
    def _handle_application_message(self, link: PeerLink, content: ApplicationData) -> None:
    ...
    def _send(self, link: PeerLink, data: bytes) -> None:
        if SEPARATOR in data: raise P2PRuntimeError(f"found separator {SEPARATOR!r} in data")
        link.socket.sendall(data + SEPARATOR)
    def _recv(self, link: PeerLink) -> None:
        chunk = link.socket.recv(NODE_CHUNK_SIZE)
        if not chunk:
            link.alive = False
            if link.socket: link.socket.close()
            return
        link.buffer += chunk
