...
USER_AGENT = "AxiomNetwork Research AxiomServer/0.5 (victornevarez88@gmail.com)"
...
def get_financial_facts_from_edgar(max_filings: int = 10) -> list[dict]:
    ...
    if not USER_AGENT or "YourName" in USER_AGENT:
        logger.warning("SEC EDGAR agent is not configured with a proper User-Agent. Skipping.")
        return []
    ...
    for filing in recent_filings[:max_filings * 5]: # Fetch more to find enough with data
        ...
        try:
            ...
            if not hasattr(edgar_client, "get_facts"):
                logger.debug("SEC EDGAR client has no get_facts(); skipping details for %s", ticker)
                continue
            ...
            if 'val' in revenue_data and 'val' in net_income_data:
                ...
                if not period_end_date: continue
                ...
                source_url = f"https://www.sec.gov/edgar/browse/?CIK={filing['cik']}"

                extracted_facts.append({"content": revenue_fact, "source_url": source_url})
                extracted_facts.append({"content": net_income_fact, "source_url": source_url})
            ...
            logger.debug(f"Could not extract standard financial facts for {ticker}. Skipping.")
            continue
            
    logger.info(f"Successfully extracted {len(extracted_facts)} financial facts from SEC EDGAR.")
