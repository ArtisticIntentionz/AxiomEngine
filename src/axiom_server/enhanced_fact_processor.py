...
from axiom_server.dispute_system import DisputeEvidence, DisputeSystem
...
class EnhancedFactProcessor:
    ...
    def __init__(self, node_id: str):
        ...
        self.auto_dispute_threshold = (
            0.3  # If confidence is below this, auto-create dispute
        )
    ...
    def process_fact(
        ...
        enhanced_analysis = self._perform_enhanced_analysis(
            fact, neural_result, corroborations, citations,
        )

        # Step 4: Determine Final Status
        final_status = self._determine_final_status(
            neural_result, enhanced_analysis,
        )
        ...
        if neural_result["confidence"] < self.auto_dispute_threshold:
            dispute_created = self._create_auto_dispute(
                fact, neural_result, enhanced_analysis,
            )
        ...
        fact.metadata["processing_timestamp"] = datetime.now(
            timezone.utc,
        ).isoformat()
        fact.metadata["processor_node"] = self.node_id

        if dispute_created:
            fact.metadata["auto_dispute_created"] = dispute_created[
                "dispute_id"
            ]
        ...
        logger.info(
            f"Processed fact {fact.id} with status {fact.status.value}",
        )
    ...
    def _perform_enhanced_analysis(
        ...
        analysis = {
            ...
            "corroborations": {
                ...
                "avg_similarity": sum(c["similarity"] for c in corroborations)
                / max(len(corroborations), 1),
                "sources": list(set(c["source"] for c in corroborations)),
            },
            "citations": {
                "count": len(citations),
                "valid_count": sum(
                    1 for c in citations if c.get("status") == "valid"
                ),
                "invalid_count": sum(
                    1 for c in citations if c.get("status") == "invalid"
                ),
            },
        ...
        corrob_strength = min(
            len(corroborations) * 0.1
            + analysis["corroborations"]["avg_similarity"] * 0.2,
            1.0,
        )
        confidence_factors.append(corrob_strength * 0.3)

        # Citation validity (weight: 0.2)
        citation_strength = analysis["citations"]["valid_count"] / max(
            analysis["citations"]["count"], 1,
        )
    ...
    def _analyze_sources(
        ...
        reliable_count = sum(
            1 for domain in domains if domain in reliable_domains
        )
    ...
    def _determine_final_status(
        self, neural_result: Dict[str, Any], enhanced_analysis: Dict[str, Any],
    ) -> FactStatus:
    ...
    def _create_auto_dispute(
        ...
        evidence = DisputeEvidence(
            ...
            - neural_result[
                "confidence"
            ],  # Inverse confidence as evidence strength
        )

        dispute = self.dispute_system.create_dispute(
            fact_id=fact.id, reason=reason, evidence=[evidence],
        )
        ...
        logger.info(
            f"Auto-created dispute {dispute.dispute_id} for fact {fact.id}",
        )
    ...
    def review_fact(
        ...
        with SessionMaker() as session:
            ...
            if review_decision == "dispute":
                ...
                # Broadcast dispute
                broadcast_result = self.dispute_system.broadcast_dispute(
                    dispute,
                )
    ...
    def get_processing_statistics(self) -> Dict[str, Any]:
        ...
        disputes_created = sum(
            1 for r in self.processing_history if r.get("dispute_created")
        )
    ...
    def train_neural_verifier(
        self, training_data: List[Tuple[str, int]],
    ) -> Dict[str, Any]:
        ...
        with SessionMaker() as session:
            for fact_content, label in training_data:
                ...
                fact = (
                    session.query(Fact)
                    .filter(Fact.content == fact_content)
                    .first()
                )
                if not fact:
                    fact = Fact(
                        content=fact_content, status=FactStatus.INGESTED,
                    )
                    session.add(fact)
                    session.commit()
