...
class Dispute(Base):
    ...
    status: Mapped[str] = mapped_column(
        String, nullable=False, default=DisputeStatus.OPENED,
    )
    ...
    consensus_threshold: Mapped[str] = mapped_column(
        String, default="0.75",
    )  # 75% agreement required
    neural_verification_enabled: Mapped[bool] = mapped_column(
...
class DisputeSystem:
    ...
    def __init__(
        self,
        node_id: str,
        neural_verifier: Optional[NeuralFactVerifier] = None,
    ):
    ...
    def create_dispute(
        ...
        fact_id: str,
        reason: str,
        evidence: Optional[List[DisputeEvidence]] = None,
    ) -> DisputeData:
        """Create a new dispute against a fact."""
        dispute_id = str(uuid4())
        expires_at = datetime.now(timezone.utc) + timedelta(
            hours=self.dispute_timeout_hours,
        )
    ...
    def add_evidence(self, dispute_id: str, evidence: DisputeEvidence) -> bool:
        ...
        if dispute.status != DisputeStatus.OPENED:
            logger.warning(
                f"Cannot add evidence to dispute {dispute_id} with status {dispute.status}",
            )
            return False
    ...
    def cast_vote(
        ...
        if dispute.status not in [
            ...
            logger.warning(
                f"Cannot vote on dispute {dispute_id} with status {dispute.status}",
            )
            return False

        # Check if this node has already voted
        existing_vote = next(
            (v for v in dispute.votes if v.node_id == self.node_id), None,
        )
        if existing_vote:
            logger.warning(
                f"Node {self.node_id} has already voted on dispute {dispute_id}",
            )
            return False
        ...
        logger.info(
            f"Cast vote on dispute {dispute_id}: {vote} (confidence: {confidence})",
        )
        return True
    ...
    def _check_dispute_resolution(self, dispute: DisputeData) -> None:
        ...
        if false_ratio >= consensus_threshold:
            ...
            logger.info(
                f"Dispute {dispute.dispute_id} resolved: FACT IS FALSE",
            )
    ...
    def resolve_dispute(
        self, dispute_id: str, resolution: DisputeStatus, reason: str,
    ) -> bool:
        ...
        if dispute.status in [
            DisputeStatus.RESOLVED_TRUE,
            DisputeStatus.RESOLVED_FALSE,
    ...
    def check_expired_disputes(self) -> List[str]:
        ...
        for dispute_id, dispute in self.active_disputes.items():
            if (
                dispute.expires_at < current_time
                and dispute.status == DisputeStatus.OPENED
            ):
    ...
    def get_dispute_summary(self, dispute_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "time_remaining": (
                dispute.expires_at - datetime.now(timezone.utc)
            ).total_seconds()
            / 3600,
        }
    ...
    def get_active_disputes(self) -> List[Dict[str, Any]]:
        ...
        return [
            self.get_dispute_summary(did)
            for did in self.active_disputes.keys()
        ]
    ...
    def broadcast_dispute(self, dispute: Dispute) -> Dict[str, Any]:
        ...
        logger.info(
            f"Broadcasting dispute {dispute.dispute_id} to P2P network",
        )
        ...
        try:
            ...
            dispute = DisputeData(
                ...
                expires_at=datetime.fromisoformat(
                    broadcast_data["expires_at"],
                ),
                evidence=[
                    DisputeEvidence(**ev)
                    for ev in broadcast_data.get("evidence", [])
                ],
                votes=[],
    ...
    def apply_dispute_resolution(
        self, dispute: DisputeData, ledger_session,
    ) -> bool:
        ...
        try:
            ...
            fact = (
                ledger_session.query(Fact)
                .filter(Fact.id == dispute.fact_id)
                .first()
            )
            if not fact:
            ...
            if dispute.status == DisputeStatus.RESOLVED_FALSE:
                ...
                fact.metadata["dispute_resolved_at"] = datetime.now(
                    timezone.utc,
                ).isoformat()
                ...
                logger.info(
                    f"Fact {dispute.fact_id} confirmed as true through dispute resolution",
                )
    ...
    def get_dispute_statistics(self) -> Dict[str, Any]:
        ...
        open_disputes = sum(
            ...
            if d.status == DisputeStatus.OPENED
        )
        resolved_false = sum(
            1
            for d in self.active_disputes.values()
            if d.status == DisputeStatus.RESOLVED_FALSE
        )
        resolved_true = sum(
            1
            for d in self.active_disputes.values()
            if d.status == DisputeStatus.RESOLVED_TRUE
        )
        expired_disputes = sum(
            1
            for d in self.active_disputes.values()
        ...
        total_votes = sum(
            d.current_votes for d in self.active_disputes.values()
        )
