...
def _get_llm_instance() -> Llama | None:
    """Loads and returns the singleton Llama model instance with enhanced error handling."""
    global llm_instance
    
    # Check if LLM is disabled via configuration
    if DISABLE_LLM:
        logger.info("LLM disabled via configuration. Using fallback mode only.")
        return None
    ...
    if llm_instance is None:
        ...
        try:
            ...
            logger.warning("LLM functionality will be disabled. Using fallback mode only.")
            return None
    return llm_instance
...
def _extract_keywords_fallback(user_query: str) -> str:
    ...
    question_words = ['what', 'when', 'where', 'who', 'why', 'how', 'is', 'are', 'was', 'were', 'do', 'does', 'did', 'can', 'could', 'will', 'would', 'should', 'the', 'a', 'an', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by']
...
def validate_user_query(user_query: str) -> Tuple[bool, str]:
    """Validates user input for potentially malicious content."""
...
def validate_llm_prompt(llm_prompt: str) -> Tuple[bool, str]:
    """Validates LLM-generated search prompt for safety."""
...
def validate_llm_response(
    ...
    """Validates LLM response against original facts to prevent hallucination."""
    if not llm_response or len(llm_response.strip()) < 10:
        return False, "Response too short"

    # Extract key information from facts
    fact_content = " ".join(
        [fact.get("content", "") for fact in original_facts],
    )
    fact_sources = " ".join(
    ...
    if fact_sources and "source" in response_lower:
        if not any(
...
def translate_user_query(user_query: str) -> Tuple[bool, str, str]:
    """Securely translates user query into Axiom search terms."""
    ...
    if llm is None:
        ...
        return True, "Fallback keyword extraction", _extract_keywords_fallback(user_query)
...
def synthesize_secure_answer(
    ...
    """Securely synthesizes natural answers from verified facts."""
...
def _create_fallback_response(facts: List[Dict[str, Any]]) -> str:
    """Creates a simple fallback response when synthesis fails."""
    if not facts:
        return (
            "I don't have enough verified information in the Axiom ledger to answer your question."
        )
    ...
    sorted_facts = sorted(facts, key=lambda x: x.get("similarity", 0), reverse=True)
    
    response = "Based on the verified facts in the Axiom ledger:\n\n"
    for i, fact in enumerate(sorted_facts[:3], 1):  # Limit to top 3 most relevant
        content = fact.get("content", "No content available.")
        sources = ", ".join(fact.get("sources", ["Unknown source"]))
...
def process_user_query(
    ...
    """Main secure interface for processing user queries with facts."""
