...
INPUT_HASH_FILE = "/Users/vic/Desktop/hashes.txt" # <-- Make sure this path is correct
...
def get_hashes_from_file(filepath):
    ...
    try:
        ...
        if not hashes:
            ...
            return []
        print(f"Successfully read and cleaned {len(hashes)} hash(es) from {filepath}.")
        return hashes
...
def process_and_save_facts(api_data):
    ...
    if not list_of_facts:
        print("Error: Could not find a list of fact objects in the API response.")
        return
    ...
    try:
        with open(output_filepath, 'w', encoding='utf-8') as f:
            ...
            def write_section(title, facts_list):
                f.write(f"--- {title.upper()} (Total: {len(facts_list)}) ---\n\n")
                if not facts_list:
                    f.write("None found.\n\n")
                else:
            ...
            write_section("Verified or Corroborated Facts (Score > 0)", verified_facts)
