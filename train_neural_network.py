...
def train_neural_network():
    ...
    with SessionMaker() as session:
        for i, data in enumerate(training_data):
            logger.info(f"Training fact {i+1}/{len(training_data)}: {data['content'][:50]}...")
            ...
            try:
                ...
                # Log the result
                logger.info(f"  - Neural confidence: {result['confidence']:.3f}")
                logger.info(f"  - Expected: {data['is_true']}, Got: {result['verified']}")
...
def main():
    ...
    try:
        ...
        logger.info("✅ The neural network now has training data and should perform better")
        logger.info("✅ You can now use the web interface with improved neural verification")
        logger.info("✅ The system will continue to learn from new facts over time")
