...
def test_neural_endpoints():
    ...
    try:
        response = requests.post(f"{base_url}/neural/verify_fact", json=test_fact)
        if response.status_code == 200:
            result = response.json()
            print("✅ Neural verification endpoint working!")
            print(f"   Confidence: {result['verification_result']['confidence']:.3f}")
    ...
    try:
        response = requests.post(f"{base_url}/neural/process_fact", json=test_fact_processing)
        if response.status_code == 200:
            result = response.json()
            print("✅ Enhanced fact processing endpoint working!")
            print(f"   Status: {result['processing_result']['status']}")
            print(f"   Neural Confidence: {result['processing_result']['neural_verification']['confidence']:.3f}")
        else:
            print(f"❌ Enhanced fact processing failed: {response.status_code}")
            print(f"   Response: {response.text}")
    except Exception as e:
    ...
    try:
        response = requests.post(f"{base_url}/dispute/create", json=test_dispute)
        if response.status_code == 200:
            result = response.json()
...
def test_status_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Total disputes: {result['statistics']['total_disputes']}")
...
def main():
    ...
    print("If you see ✅ marks, the neural network system is working in the node!")
    print("=" * 60)
